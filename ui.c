/* ui.c: Ncurses rendering, input handling, and menu flows for all gameplay
 * screens. */
#include "ui.h"
...
static const npc_dialogue_t HEAL_NPCS[MAX_LOCATIONS] = {
    [LOCATION_STARPORT] = {"Dr. Ananya Patel",
                           "I treat everyone. That doesn't mean I have to "
...
static const npc_dialogue_t TRADE_NPCS[MAX_LOCATIONS] = {
    [LOCATION_STARPORT] =
        {"Juno Creed",
         ...
         "Sharp-eyed market broker with a ledger memory and no patience for "
         "sloppy deals."},
    [LOCATION_ASHFIELD] =
        {"Dottie Marsh",
         "I run a clean shop. You cause trouble, you take it elsewhere.",
         "Looking for something in particular, or just browsing?",
         "I kept a few things back since I thought you might be coming "
         "through. Have a look.",
         "Practical storekeeper who has run Marsh Provisions long enough to "
    ...
    [LOCATION_BROKENHILL] =
        {"Gal Okonkwo",
         ...
         "Always good to deal with someone who knows the value of things. I'll "
         "see what I can do on the price.",
         "Disciplined trader who built Brokenhill's strongest supply line from "
         "one ore cart."},
    [LOCATION_COLDWATER] =
        {"Tomasz Wick",
         "Coldwater's a trading town. You want something, you pay for it. "
         "Simple.",
         "Good day. Stock's fresh in. The board shows what we have.",
         "Ah, my favourite customer. Let me show you what came in on the last "
         "run.",
         "Third-generation merchant who treats price discovery like a craft."},
    [LOCATION_MILLHAVEN] = {
        "Pell",
        "We trade with everyone. The collective voted on it. Doesn't mean I "
        "have to enjoy it.",
        "Welcome to the collective store. We have what we have.",
        "Oh good, it's you. I kept some of the good grain aside - I thought "
        "you might be passing through.",
        "Millhaven's rotation trader: earnest, fair, and strict about "
        "collective pricing."}};
 ...
 *   - location (location_id_t): Location identifier used to select travel or
 * market context. Returns:
 *   - const npc_dialogue_t *: Pointer result selected by this routine.
 */
static const npc_dialogue_t *npc_dialogue_for(const npc_dialogue_t *dialogues,
 ...
 */
static const char *npc_dialogue_line(const npc_dialogue_t *npc,
                                     int reputation) {
  if (npc == NULL) {
    return "";
  }
...
static void format_npc_speaker(const npc_dialogue_t *npc,
                               const char *profession, char *buffer,
                               size_t buffer_size) {
 ...
 *   - buffer_size (size_t): Output buffer populated by this routine.
 */
static void format_npc_quote(const char *line, char *buffer,
                             size_t buffer_size) {
  if (buffer == NULL || buffer_size == 0) {
    return;
  }
 ...
 * Parameters:
 *   - npc (const npc_dialogue_t *): Input argument used by this routine.
 *   - fallback_role (const char *): Text input used for display, messaging, or
 * formatting.
 *   - buffer (char *): Output buffer populated by this routine.
 *   - buffer_size (size_t): Output buffer populated by this routine.
 */
static void format_npc_profile(const npc_dialogue_t *npc,
                               const char *fallback_role, char *buffer,
                               size_t buffer_size) {
  if (buffer == NULL || buffer_size == 0) {
    return;
  }
  ...
  if (npc != NULL && npc->name != NULL && npc->name[0] != '\0') {
    snprintf(buffer, buffer_size, "%s is waiting behind the counter.",
             npc->name);
    return;
  }

  snprintf(buffer, buffer_size, "A local %s is waiting behind the counter.",
           fallback_role);
}
 ...
 *   - None.
 * Returns:
 *   - bool: True when the operation succeeds or the condition is met; false
 * otherwise.
 */
...
static void draw_terminal_size_warning(void) {
  ...
  mvwprintw(stdscr, row + 4, col,
            "The game redraws automatically on SIGWINCH.");
  refresh();
}
 ...
 * Parameters:
 *   - value (int): Numeric input used by this routine for calculations or
 * limits.
 ...
 *   - int: Computed numeric result for this routine.
 */
static int ui_scale_w(int w) { return ui_scale_span(w, UI_BASE_COLS, COLS); }
 ...
 *   - y (int): Input argument used by this routine.
 *   - x (int): Input argument used by this routine.
 *   - text (const char *): Text input used for display, messaging, or
 * formatting.
 ...
 *   - y (int): Input argument used by this routine.
 *   - x (int): Input argument used by this routine.
 *   - fmt (const char *): Text input used for display, messaging, or
 * formatting.
 ...
 *   - ch (chtype): Input argument used by this routine.
 */
static void ui_draw_vertical_separator(int x, int y_start, int y_end,
                                       chtype ch) {
  int sx = ui_scale_x(x);
 ...
 *   - ch (chtype): Input argument used by this routine.
 */
static void ui_draw_horizontal_separator(int y, int x_start, int x_end,
                                         chtype ch) {
  int sy = ui_scale_y(y);
 ...
 *   - char lines[][LOG_LINE_LENGTH] (char lines[][LOG_LINE_LENGTH]): Input
 * argument used by this routine.
 *   - max_lines (int): Input argument used by this routine.
 * Returns:
 *   - int: Computed numeric result for this routine.
 */
static int wrap_text_lines(const char *text, int width,
                           char lines[][LOG_LINE_LENGTH], int max_lines) {
 ...
 */
static void draw_clipped_multiline(const char *text, int row, int col,
 ...
 *   - int: Computed numeric result for this routine.
 */
static int draw_wrapped_text(const char *text, int row, int col, int width,
...
static void draw_inventory_panel(const game_t *game) {
  ...
  mvprintw(2, RIGHT_X, "Inventory (%d.%d/%d.%d u)", used / 10, used % 10,
  ...
  for (int i = 0; i < game->player.cargo_count && i < 9; ++i) {
    ...
    mvprintw(3 + i, RIGHT_X, "%-17s x%-3d %2d.%d u",
             COMMODITIES[stack->commodity].name, stack->quantity, units / 10,
             units % 10);
  }
...
static void draw_log_panel(const game_t *game) {
  ...
  if (available_rows >= 4) {
    ...
    for (int i = 0; i < shown && (top_row + 1 + i) < LINES; ++i) {
      int index =
          (game->log.head - shown + i + NEWS_TICKER_LINES) % NEWS_TICKER_LINES;
  ...
  for (int i = 0; i < shown; ++i) {
    int index =
        (game->log.head - shown + i + NEWS_TICKER_LINES) % NEWS_TICKER_LINES;
    mvwprintw(stdscr, top_row + i, col, i == 0 ? "Log: > %s" : "> %s",
              game->log.text[index]);
  }
}
 ...
 * Parameters:
 *   - game (const game_t *): Game state this routine reads and/or updates.
 *   - commands (const char *): Text input used for display, messaging, or
 * formatting.
 */
...
static int draw_location_view(const game_t *game, int description_scroll) {
  ...
  if (location->kind == LOCATION_KIND_WILDERNESS) {
    ...
    snprintf(commands, sizeof(commands),
             "[1-4]Travel [M]arket [B]ar [H]ospital [N]otice [I]nv%s [X]Pay "
             "[V]Save [Q]uit",
             ground_option);
  } else {
    snprintf(commands, sizeof(commands),
             "[1-4]Travel [S]tore [B]ar [C]linic [I]nventory%s [V]Save [Q]uit",
             ground_option);
  }
  ...
  description_lines = draw_wrapped_text(location->description, 9, LEFT_TEXT_X,
                                        LEFT_TEXT_WIDTH, 3, description_scroll);
  ...
  for (int i = 0; i < location->neighbor_count; ++i) {
    mvprintw(14 + i, 4, "%d. %s", i + 1,
             LOCATIONS[location->neighbors[i]].name);
  }
  if (has_ground) {
...
static void show_notice_board(game_t *game) {
  ...
  while (running) {
    ...
    for (int i = 0; i < MAX_LOCATIONS; ++i) {
      if (!market_has_open_market((location_id_t)i) ||
          !game->markets[i].known) {
        continue;
      }
      mvprintw(6 + i, 4, "%-11s %4d cr  stock %2d", LOCATIONS[i].name,
...
static void inventory_menu(game_t *game) {
  ...
  while (running) {
    draw_base_frame(game, "[Up/Down] select  [D]rop one  [A]drop all  "
                          "[B]andage  [M]edkit  [Esc]");
    mvprintw(2, 2, "Inventory");
    if (game->player.cargo_count == 0) {
      mvprintw(4, 2, "No cargo.");
    }
    for (int i = 0; i < game->player.cargo_count; ++i) {
      int units = COMMODITIES[game->player.cargo[i].commodity].units_tenths *
      ...
      mvprintw(4 + i, 2, "%-20s x%-3d %2d.%d u",
               ...
               game->player.cargo[i].quantity, units / 10, units % 10);
      if (i == selected) {
        attroff(A_REVERSE);
      }
    }
    ...
    case 'A':
      if (game->player.cargo_count > 0) {
        ...
        world_drop_commodity(game, game->player.location, stack.commodity,
                             stack.quantity);
        game_log(game, "Dropped %d x %s.", stack.quantity,
    ...
    for (int i = 0; i < MAX_DROP_STACKS_PER_LOCATION; ++i) {
      ...
      if (slot->occupied) {
        if (slot->kind == DROP_KIND_CART) {
          ...
          mvprintw(4 + i, 2, "%-20s x%-3d", COMMODITIES[slot->commodity].name,
                   slot->quantity);
        }
      } else {
...
static void trade_menu(game_t *game, bool fence) {
  ...
  const npc_dialogue_t *trade_npc =
      npc_dialogue_for(TRADE_NPCS, game->player.location);
  const char *trade_line =
      npc_dialogue_line(trade_npc, game->player.reputation);
  ...
  for (int i = 0; i < NUM_COMMODITIES; ++i) {
    bool show =
        fence ? (i == COMMODITY_NARCOTICS || i == COMMODITY_STOLEN_GOODS ||
                 i == COMMODITY_ARTIFACTS)
  ...
  while (running) {
    ...
    mvprintw(list_count_row, 2, "Showing %d-%d of %d",
             ...
             option_count == 0 ? 0
                               : clamp_int(top + list_height, 0, option_count),
             option_count);
    ...
    for (int row = 0; row < list_height && (top + row) < option_count; ++row) {
      ...
      if (!fence) {
        price = market_can_sell_openly(game->player.location,
                                       (commodity_id_t)commodity_index)
                    ? market_sell_price(game, game->player.location,
                                        (commodity_id_t)commodity_index)
                    : market_buy_price(game, game->player.location,
                                       (commodity_id_t)commodity_index);
    ...
    case 'a':
    case 'A': {
      int qty = player_cargo_quantity(&game->player,
...
static void store_menu(game_t *game) {
  ...
  const npc_dialogue_t *store_npc =
      npc_dialogue_for(TRADE_NPCS, game->player.location);
  const char *store_line =
      npc_dialogue_line(store_npc, game->player.reputation);
  ...
  while (running) {
    ...
    mvprintw(2, 2,
             game->player.location == LOCATION_STARPORT ? "Starport Market Gear"
                                                        : "Settlement Store");
    ...
    mvprintw(6, 2, "Showing %d-%d of %d", option_count == 0 ? 0 : (top + 1),
             option_count == 0 ? 0
                               : clamp_int(top + list_height, 0, option_count),
             option_count);
    for (int row = 0; row < list_height; ++row) {
    ...
    for (int row = 0; row < list_height && (top + row) < option_count; ++row) {
      ...
      snprintf(line, sizeof(line), "%-18s %4d cr%s", SHOP_ITEMS[item].name,
               (game->player.reputation >= 2
                    ? (SHOP_ITEMS[item].cost * 95) / 100
                    : SHOP_ITEMS[item].cost),
               player_owns_shop_item(&game->player, (shop_item_id_t)item)
                   ? " [owned]"
                   : "");
      if ((top + row) == selected) {
        attron(A_REVERSE);
    ...
    if (option_count > 0) {
      draw_wrapped_text(SHOP_ITEMS[options[selected]].description, 15,
                        LEFT_TEXT_X, LEFT_TEXT_WIDTH, 4, 0);
    } else {
      draw_wrapped_text("No items available.", 15, LEFT_TEXT_X, LEFT_TEXT_WIDTH,
                        4, 0);
    }
    refresh();
 ...
 *   - limit (int): Numeric input used by this routine for calculations or
 * limits.
 */
...
static void atm_menu(game_t *game) {
  ...
  while (running) {
    ...
    draw_base_frame(
        game, "[D]eposit [W]ithdraw [Enter] confirm [+/-/0-9] amount [Esc]");
    ...
    mvprintw(7, 2, "Selected action : %s",
    ...
    if (limit <= 0) {
      ...
      mvprintw(
          10, 2,
          "Balances below 100 cr earn 0 interest this turn (floor rounding).");
    }
    mvprintw(
        12, 2,
        "Press D or W to execute immediately, or Enter for selected action.");
    refresh();
...
static void trade_hub_menu(game_t *game) {
  ...
  format_npc_speaker(trade_npc, "shopkeeper", trade_speaker,
                     sizeof(trade_speaker));
  format_npc_profile(trade_npc, "shopkeeper", trade_profile,
                     sizeof(trade_profile));
  format_npc_quote(trade_line, trade_quote, sizeof(trade_quote));

  while (running) {
    int ch;
    const char *title = game->player.location == LOCATION_STARPORT
                            ? "Starport Market"
                            : "Settlement Store";

    draw_base_frame(game,
                    "[1]Gear [2]Commodities [3]ATM [Up/Down] [Enter] [Esc]");
 ...
 */
static void bet_input_clear(bet_input_t *input) { input->digits[0] = '\0'; }
 ...
 */
static void adjust_bet_from_input(int *bet, const bet_input_t *input,
                                  int min_bet, int max_allowed) {
  int parsed;
 ...
 *   - credits (int): Input argument used by this routine.
 */
static void handle_bet_key(int key, int *bet, bet_input_t *input, int min_bet,
                           int max_bet, int credits) {
 ...
 *   - card_count (int): Numeric input used by this routine for calculations or
 * limits.
 *   - hide_hole (bool): Boolean flag controlling conditional behavior.
 *   - buffer (char *): Output buffer populated by this routine.
 *   - buffer_size (size_t): Output buffer populated by this routine.
 */
static void format_blackjack_hand(const int *cards, int card_count,
                                  bool hide_hole, char *buffer,
                                  size_t buffer_size) {
  size_t used = 0;
 ...
 *   - dealer_count (int *): Numeric input used by this routine for calculations
 * or limits.
 *   - result_line (char *): Input argument used by this routine.
 *   - result_line_size (size_t): Capacity value, typically in bytes, for the
 * associated buffer or container. Returns:
 *   - int: Computed numeric result for this routine.
 */
static int settle_blackjack_round(game_t *game, int wager,
                                  const int *player_cards, int player_count,
                                  int *dealer_cards, int *dealer_count,
                                  char *result_line, size_t result_line_size) {
  ...
  if (player_total > 21) {
    snprintf(result_line, result_line_size, "You bust at %d. You lose %d cr.",
             player_total, wager);
    return 0;
  }
  ...
  if (dealer_total > 21) {
    snprintf(result_line, result_line_size,
             "Dealer busts at %d. You win %d cr.", dealer_total, wager);
    return wager * 2;
  }
  if (player_total > dealer_total) {
    snprintf(result_line, result_line_size, "You win %d to %d. +%d cr.",
             player_total, dealer_total, wager);
    return wager * 2;
  }
  if (player_total == dealer_total) {
    snprintf(result_line, result_line_size, "Push at %d. Bet returned.",
             player_total);
    return wager;
  }

  snprintf(result_line, result_line_size,
           "Dealer wins %d to %d. You lose %d cr.", dealer_total, player_total,
           wager);
  return 0;
}
...
static void blackjack_menu(game_t *game) {
  ...
  while (running) {
    ...
    draw_base_frame(game, state == BLACKJACK_BETTING
                              ? "[+/-/0-9] bet  [Enter] deal  [Esc]"
                              : (state == BLACKJACK_PLAYING
                                     ? "[H]it [S]tand [D]ouble [Esc]"
                                     : "[Enter] next hand  [Esc]"));
    ...
    if (state == BLACKJACK_BETTING) {
      ...
      if (!can_play) {
        mvprintw(10, 2, "You need at least %d cr to sit at this table.",
                 min_bet);
      } else {
        mvprintw(10, 2, "Press Enter to deal.");
      }
    } else {
      format_blackjack_hand(dealer_cards, dealer_count, hide_hole, dealer_hand,
                            sizeof(dealer_hand));
      format_blackjack_hand(player_cards, player_count, false, player_hand,
                            sizeof(player_hand));
...

    {
      ...
      case 'Q':
        if (state == BLACKJACK_PLAYING) {
          snprintf(result_line, sizeof(result_line),
                   "Finish the current hand before leaving the table.");
        } else {
          running = 0;
        }
        break;
      ...
      case KEY_ENTER:
        if (state == BLACKJACK_BETTING) {
          ...
          player_blackjack =
              gamble_blackjack_is_natural(player_cards, player_count);
          dealer_blackjack =
              gamble_blackjack_is_natural(dealer_cards, dealer_count);
          if (player_blackjack || dealer_blackjack) {
            if (player_blackjack && dealer_blackjack) {
              payout = wager;
              snprintf(result_line, sizeof(result_line),
                       "Both players have blackjack. Push.");
            } else if (player_blackjack) {
              payout = (wager * 5) / 2;
              snprintf(result_line, sizeof(result_line),
                       "Blackjack! You win %d cr.", (payout - wager));
            } else {
              snprintf(result_line, sizeof(result_line),
                       "Dealer has blackjack. You lose %d cr.", wager);
            }
        ...
        if (state == BLACKJACK_PLAYING) {
          if (player_count < 12) {
            ...
            if (gamble_blackjack_value(player_cards, player_count, NULL) > 21) {
              payout = settle_blackjack_round(
                  game, wager, player_cards, player_count, dealer_cards,
                  &dealer_count, result_line, sizeof(result_line));
              game->player.credits += payout;
              game_log(game, "%s", result_line);
              state = BLACKJACK_RESULT;
            }
      ...
      case 'S':
        if (state == BLACKJACK_PLAYING) {
          int payout = settle_blackjack_round(
              game, wager, player_cards, player_count, dealer_cards,
              &dealer_count, result_line, sizeof(result_line));
          game->player.credits += payout;
          game_log(game, "%s", result_line);
      ...
      case 'D':
        if (state == BLACKJACK_PLAYING) {
          if (!gamble_blackjack_can_double(player_count, first_action)) {
            snprintf(result_line, sizeof(result_line),
                     "Double is only available as your first action.");
          } else if (game->player.credits < wager) {
            snprintf(result_line, sizeof(result_line),
                     "Not enough credits to double this hand.");
          } else {
            int payout;
      ...
      default:
        if (state == BLACKJACK_BETTING) {
          handle_bet_key(key, &bet, &bet_input, min_bet, max_bet,
                         game->player.credits);
        }
        break;
      }
 ...
 *   - straight_value (int): Numeric input used by this routine for calculations
 * or limits.
 *   - color_value (int): Numeric input used by this routine for calculations or
 * limits.
 *   - parity_value (int): Numeric input used by this routine for calculations
 * or limits.
 *   - range_value (int): Numeric input used by this routine for calculations or
 * limits.
 *   - dozen_value (int): Numeric input used by this routine for calculations or
 * limits.
 *   - column_value (int): Numeric input used by this routine for calculations
 * or limits. Returns:
 *   - roulette_bet_t: Return value describing the outcome of this routine.
 */
static roulette_bet_t build_roulette_bet(int selected, int straight_value,
                                         int color_value, int parity_value,
                                         int range_value, int dozen_value,
                                         int column_value) {
...
static void roulette_menu(game_t *game) {
  ...
  while (running) {
    int max_allowed = cap_bet_by_credits(max_bet, game->player.credits);
    bool can_play = max_allowed >= min_bet;
    roulette_bet_t active_bet =
        build_roulette_bet(selected, straight_value, color_value, parity_value,
                           range_value, dozen_value, column_value);
    char active_bet_text[64];

    gamble_roulette_describe_bet(&active_bet, active_bet_text,
                                 sizeof(active_bet_text));

    draw_base_frame(game, "[Up/Down] type [Left/Right] option [+/-/0-9] wager "
                          "[Enter] spin [Esc]");
    mvprintw(2, 2, "Roulette Table");
    mvprintw(3, 2, "European wheel (0-36). Max wager per spin: %d cr.",
             max_bet);
    ...
    for (int row = 0; row < 6; ++row) {
      ...
      case 0:
        ...
        break;
      case 1:
        mvprintw(5 + row, 2, "Color: %-5s (1:1)",
                 color_value == ROULETTE_COLOR_RED ? "Red" : "Black");
        break;
      case 2:
        mvprintw(5 + row, 2, "Odd/Even: %-4s (1:1)",
                 parity_value == 0 ? "Odd" : "Even");
        break;
      case 3:
        mvprintw(5 + row, 2, "Low/High: %-5s (1:1)",
                 range_value == 0 ? "1-18" : "19-36");
        break;
      case 4:
        mvprintw(5 + row, 2, "Dozen: %-6s (2:1)",
                 dozen_value == 0 ? "1st 12"
                                  : (dozen_value == 1 ? "2nd 12" : "3rd 12"));
        break;
      default:
        mvprintw(5 + row, 2, "Column: %-5s (2:1)",
                 column_value == 0 ? "1st"
                                   : (column_value == 1 ? "2nd" : "3rd"));
        break;
      }
      if (row == selected) {
        attroff(A_REVERSE);
    ...
    if (!can_play) {
      mvprintw(15, 2, "You need at least %d credits to place a wager.",
               min_bet);
    } else {
      mvhline(15, 2, ' ', LEFT_TEXT_WIDTH);
    }
...

    {
      ...
      case KEY_LEFT:
        ...
        case 0:
          ...
          break;
        case 1:
          color_value = color_value == ROULETTE_COLOR_RED ? ROULETTE_COLOR_BLACK
                                                          : ROULETTE_COLOR_RED;
          break;
        case 2:
          parity_value ^= 1;
      ...
      case KEY_ENTER:
        if (can_play) {
          ...
          roulette_bet_t settled_bet = build_roulette_bet(
              selected, straight_value, color_value, parity_value, range_value,
              dozen_value, column_value);
          ...
          if (won) {
            payout =
                wager * (gamble_roulette_profit_multiplier(&settled_bet) + 1);
          }
          game->player.credits += payout;
          net = payout - wager;
          color_name = spin == 0
                           ? "green"
                           : (gamble_roulette_is_red(spin) ? "red" : "black");
          gamble_roulette_describe_bet(&settled_bet, settled_bet_text,
                                       sizeof(settled_bet_text));

          if (won) {
            snprintf(last_result, sizeof(last_result),
                     "Wheel: %d (%s). You win %d cr on %s.", spin, color_name,
                     net, settled_bet_text);
          } else {
            snprintf(last_result, sizeof(last_result),
                     "Wheel: %d (%s). You lose %d cr on %s.", spin, color_name,
                     wager, settled_bet_text);
          }
...
static void bar_menu(game_t *game) {
  int running = 1;
  const npc_dialogue_t *bar_npc =
      npc_dialogue_for(BAR_NPCS, game->player.location);
  ...
  while (running) {
    ...
    case 'b':
    case 'B':
      if (combat_run(game, ENEMY_BAR_BRAWLER, "Bar Brawl") ==
          COMBAT_RESULT_WON) {
        int purse = rng_range(game, 10, 50);
        game->player.credits += purse;
        game->player.reputation++;
        game_log(game,
                 "You take the purse and the room's respect. +%d cr, rep +1.",
                 purse);
      }
...
static void heal_menu(game_t *game, bool hospital) {
  ...
  const npc_dialogue_t *heal_npc =
      npc_dialogue_for(HEAL_NPCS, game->player.location);
  const char *heal_line = npc_dialogue_line(heal_npc, game->player.reputation);

  format_npc_speaker(heal_npc, hospital ? "doctor" : "healer", heal_speaker,
                     sizeof(heal_speaker));
  ...
  while (running) {
    ...
    snprintf(service_text, sizeof(service_text),
             "%s price: %d cr per HP, up to %d HP per visit",
  ...
  while (running) {
    ...
    for (int i = 0; i < location->neighbor_count; ++i) {
      ...
      mvprintw(4 + i, 2, "%s (%s)", LOCATIONS[next].name,
               LOCATIONS[next].kind == LOCATION_KIND_WILDERNESS ? "wilderness"
                                                                : "settlement");
      if (i == selected) {
        attroff(A_REVERSE);
      }
...
static void title_screen(game_t *game) {
  ...
  while (running && game->running) {
    ...
    version_col =
        title_col + (((int)strlen(title) - (int)strlen(version_line)) / 2);
    if (version_col < 0) {
      version_col = 0;
    }
    ...
    for (int i = 0; i < game->score_count && i < MAX_HIGH_SCORES; ++i) {
      mvprintw(10 + i, 18, "%2d. %-12.12s %6d  %.15s", i + 1,
               game->scores[i].name, game->scores[i].score,
...
static int end_score_value(const game_t *game) {
  return game->player.credits + game->player.bank_balance +
         market_estimate_cargo_value(game) + (game->turn * 10);
}
...
static void draw_liftoff_animation(const game_t *game) {
  ...
  static const char *frames[] = {
      ...
      " ====[ LAUNCH PAD 4 - KEPLER'S REACH STARPORT ]====\n"
      "              (the pad is empty)"};
  static const char *captions[] = {"FRAME 1 - Lifting off", "FRAME 2 - Rising",
                                   "FRAME 3 - Climbing", "FRAME 4 - Departing",
                                   "FRAME 5 - Gone"};
...
static void draw_end_stats_panel(const game_t *game, bool victory) {
  ...
  int total_wealth =
      game->player.credits + game->player.bank_balance + cargo_value;
  int score = end_score_value(game);
  const char *title = victory ? "VICTORY" : "EPITAPH";
  const char *headline =
      victory ? "FREEBIRD HAS LEFT STARPORT" : "THE DUST CLAIMS ANOTHER TRADER";
  const char *farewell =
      victory ? "Fare thee well, trader. The stars are yours now!"
...
static void location_loop(game_t *game) {
  ...
  case KEY_PPAGE:
    description_scroll =
        clamp_scroll_offset(description_scroll - 1, description_lines, 3);
    break;
  case KEY_NPAGE:
    description_scroll =
        clamp_scroll_offset(description_scroll + 1, description_lines, 3);
